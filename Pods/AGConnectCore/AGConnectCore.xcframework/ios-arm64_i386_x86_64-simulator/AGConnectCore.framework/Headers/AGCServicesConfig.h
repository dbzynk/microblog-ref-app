...
typedef NS_ENUM(NSUInteger, AGCRoutePolicy) {
    AGCRoutePolicyUnknown,
    AGCRoutePolicyChina,
    AGCRoutePolicyGermany,
    AGCRoutePolicyRussia,
    AGCRoutePolicySingapore
};

/// 读取agc的配置文件信息
@interface AGCServicesConfig : NSObject <NSCopying>

/// 配置文件的唯一标识，开发者可手动设置，默认根据配置文件的配置生成
...
/// AppGallery Connect 平台分配的存储地，开发者可手动设置，默认返回plist文件中的region值
@property(nonatomic) AGCRoutePolicy routePolicy;

/// 初始化默认配置，使用agconnect-services.plist文件
- (instancetype)initWithDefaultPlist;

/// 初始化配置，使用工程根目录的plist文件
/// @param plistName 工程根目录的plist文件名
